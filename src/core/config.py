...
class AppConfig(BaseSettings):
    """Application, logging, file upload, and security configuration."""
    ...
    log_max_size: int = Field(default=10, description="Log file max size in MB")
    ...
    # Performance
    max_concurrent_uploads: int = Field(default=5, description="Maximum concurrent file uploads")
    enable_async_processing: bool = Field(default=True, description="Enable asynchronous processing")
    batch_size: int = Field(default=10, description="Batch size for processing")

    # Development
    mock_embeddings: bool = Field(default=False, description="Use mock embeddings for development")
    mock_llm_responses: bool = Field(default=False, description="Use mock LLM responses for development")
...
class LLMConfig(BaseSettings):
    """OpenAI, LlamaParse, and embeddings configuration."""
...
class RAGConfig(BaseSettings):
    """Vector store, retrieval, reranking, and query expansion configuration."""
    ...
    rerank_model: str = Field(default="BAAI/bge-reranker-large", description="Reranking model")
    use_llm_rerank: bool = Field(default=True, description="Enable LLM-based reranking")
